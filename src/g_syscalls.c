...
{
	if (!HAVEEXTENSION(G_CHANGELEVEL_HUB))
		syscall(G_CHANGELEVEL, name, entityname);	//lame fallback.
	else
...
{
	if (!HAVEEXTENSION(G_URI_QUERY))
		return 0;	//report failure instantly. no callback.
...
qbool trap_VisibleTo(intptr_t viewer, intptr_t viewee)
{
